...
{	
	...
	*/
	 ...
	 * @return void
	 */
	public function __construct($apiKey, $print_errors = true)
	{
		if($_REQUEST['mode']=='testApiConnection' && $apiKey!='' ){
			...
			$campaign_mode = $camp_campaignMeta[0]['meta_value']; 
			//camp_api_url 
			$al_key = $campaign_mode;
			$t_params['id'] = $al_key;
			...
			if(!extension_loaded('curl')) trigger_error('GetResponsePHP requires PHP cURL', E_USER_ERROR);
			if(is_null($camp_api_key)) trigger_error('API key must be supplied', E_USER_ERROR);
			//$this->apiKey = $apiKey;
			$this->errorsOn = ($print_errors) ? true : false;
		}
	}
	...
	*/
	...
	{
		if($_REQUEST['mode']=='testApiConnection'){
			$appid_params['id'] = $_REQUEST['testMode'].$_SESSION['userId'];
			$appid_params['meta_key'] = 'camp_api_key';
			$appid_appMeta = campaign::getCampaignMetaValueByCampaignId($appid_params);
			$camp_api_key = $appid_appMeta['meta_value'];			
		}	else {
	...
	private function execute($request)
	{
		...
		$camp_api_key = $appid_appMeta['meta_value'];
		if($_REQUEST['mode']=='testApiConnection'){	
			$t_params['id'] = $_REQUEST['testMode'].$_SESSION['userId'];
			$t_params['meta_key'] = 'camp_api_url';												
			$t_appMeta = campaign::getCampaignMetaValueByCampaignId($t_params);
			$camp_api_url = $t_appMeta['meta_value'];		
			$handle = curl_init($camp_api_url);		
		} else {		
			$handle = curl_init($camp_api_url);
		}
